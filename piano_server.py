...
HISTORY_DIR: /play_song が受け取ったリクエスト全体(abc/miku_notes/wav_filename等)を
30日間だけ自動で残す安全網。/save_songを呼び忘れても、この期間内なら GET /history で
一覧・中身を確認して手動で復元できる。song_libraryのような明示的な永続化ではない。

GET /history
  - 直近のリクエスト履歴(新しい順)を返す: {"history": [{"file":..., "received_at":epoch,
    "title": "abcのTフィールド|null", "has_wav": bool, "has_miku": bool}, ...]}
...
HISTORY_DIR = "/home/yunomin61/piano_history"
HISTORY_MAX_AGE_SEC = 30 * 24 * 3600  # /play_songリクエストを取りこぼしなく残す安全網の保持期間
BIND_HOST = "0.0.0.0"
BIND_PORT = 8090
...
def _record_history(data: dict) -> None:
    """/play_song が受け取ったリクエストをそのまま30日間の安全網として残す。失敗しても再生は続行する。"""
    try:
        os.makedirs(HISTORY_DIR, exist_ok=True)
        _cleanup_history()
        now = time.time()
        stamp = time.strftime("%Y%m%dT%H%M%SZ", time.gmtime(now))
        entry = {"received_at": now, **data}
        path = os.path.join(HISTORY_DIR, f"{stamp}_{uuid.uuid4().hex[:8]}.json")
...
def _extract_abc_title(abc_text: str) -> str | None:
...
def list_history() -> list[dict]:
    ...
    for fname in sorted(os.listdir(HISTORY_DIR), reverse=True):
        ...
        entries.append({
            ...
            "title": _extract_abc_title(data.get("abc") or "") or data.get("library_name") or None,
            "has_wav": bool(data.get("wav_filename")),
            "has_miku": bool(data.get("miku_notes")),
        })
    return entries
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        if self.path == "/history":
            try:
                self._send_json(200, {"history": list_history()})
            except Exception as exc:  # noqa: BLE001
                self._send_json(500, {"error": str(exc)})
            return
        self._send_json(404, {"error": "not found"})
